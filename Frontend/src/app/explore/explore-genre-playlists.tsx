...
import { GenreData, GenreDataArtists } from "../../types/deezer.types";
...
class ExplorePlaylistsClass extends React.PureComponent<InnerProps, State> {
  ...
  public render(): React.ReactNode {
    const { classes, spotifyGenre, genrePlaylists, spotifyApi, genreArtists } = this.props;
          ...
          {genrePlaylists?.playlists.items.map((playlist, index) => {
            ...
            return (
              <Grid item={true} xs={2} key={playlist.id}>
                <MappedGenrePlaylists playlist={playlist} spotifyApi={spotifyApi} />
              </Grid>
            );
          })}
        </Grid>
